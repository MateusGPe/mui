...
namespace ifd {
	...
	void FileDialog::m_renderTree(FileTreeNode* node)
	{
		...
		if (FolderNode(displayName.c_str(), (ImTextureID)(uintptr_t)m_getIcon(node->Path), isClicked)) {
			if (!node->Read) {
	...
	void FileDialog::m_renderContent()
	{
		...
		if (m_zoom == 1.0f) {
			if (ImGui::BeginTable("##contentTable", 3, /*ImGuiTableFlags_Resizable |*/ ImGuiTableFlags_Sortable, ImVec2(0, -FLT_MIN))) {
				...
				for (auto& entry : m_content) {
					...
					ImGui::Image((ImTextureID)(uintptr_t)m_getIcon(entry.Path), ImVec2(ICON_SIZE, ICON_SIZE));
					ImGui::SameLine();
		...
		// "icon" view
			...
			for (auto& entry : m_content) {
				...
				if (FileIcon(filename.c_str(), isSelected, entry.HasIconPreview ? (ImTextureID)(uintptr_t)entry.IconPreview : (ImTextureID)(uintptr_t)m_getIcon(entry.Path), ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom), entry.HasIconPreview, entry.IconPreviewWidth, entry.IconPreviewHeight)) {
					std::error_code ec;
					bool isDir = std::filesystem::is_directory(entry.Path, ec);
