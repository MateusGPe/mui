...
namespace mui
{
    ...
    void FileDialogTopBar::renderControl()
    {
        ...
        // // Reference the IconGrid zoomLevel via the controller/App state
        // if (ImGui::SliderFloat("##zoom", &zoomLevel, 1.0f, 5.0f, ""))
        // {
        //     if (onZoomChanged)
        //         onZoomChanged(zoomLevel);
        // }
        // if (ImGui::IsItemHovered())
        //     ImGui::SetTooltip("Zoom Level");
    ...
    void FileDialogBottomBar::renderControl()
    {
        ...
        if (ImGui::InputTextEx("##file_input", "", fileBuf, 1024, ImVec2(inputWidth, 0), ImGuiInputTextFlags_EnterReturnsTrue))
        {
            ...
            if (onFilenameChanged)
                onFilenameChanged(filenameInput);
            if (onOkClicked)
                onOkClicked();
        }
        ...
        if (!isDirectoryMode && !filters.empty())
        {
            ...
            {
                if (onFilterChanged)
                    onFilterChanged(selectedFilterIndex);
            }
        }
