...
namespace mui
{
    ...
    void Theme::loadSystemFont(float fontSize)
    {
        ...
        // Get the path to the Windows fonts directory, which is more robust than a
        // hardcoded path.
        ...
        { // The ideal way on Linux is to use fontconfig to find a default font.
            ...
            if (fontPath.empty() || !fileExists(fontPath))
            {
                const char *linuxFonts[] = {
                    ...
                    "/usr/share/fonts/noto/NotoSans-Regular.ttf" // Add Noto as another
                                                                 // common option
                };
        ...
        icon_config.MergeMode =
            true;                                // CRITICAL: Merges with the previously loaded font
    ...
    void Theme::applyStyle(float dpiScale)
    {
        ...
        // This flag uses a texture-based method for anti-aliasing lines. While it can
        // produce smoother lines, it has been known to cause rendering artifacts with
        // other elements, such as the 1-pixel-wide text input cursor, causing it to
        // appear white.
        ...
        colors[ImGuiCol_ChildBg] =
            ImVec4(1.00f, 1.00f, 1.00f, 1.00f); // Pure white cards
        ...
        colors[ImGuiCol_Button] =
            ImVec4(0.12f, 0.53f, 0.90f, 1.00f); // Flutter primary blue
        ...
        colors[ImGuiCol_BorderShadow] =
            ImVec4(0.00f, 0.00f, 0.00f, 0.00f); // Obsolete
    ...
    Theme::getAvailableThemes(const std::string &filepath)
    {
        ...
        try
        {
            ...
            auto findCaseInsensitive =
                [](const toml::table *table,
                   std::string_view searchKey) -> const toml::node *
            {
                if (!table)
                    return nullptr;
                for (const auto &[key, node] : *table)
                {
                    ...
                    {
                        auto kStr = key.str();
                        if (std::equal(kStr.begin(), kStr.end(), searchKey.begin(),
                                       [](char a, char b)
                                       {
    ...
    bool Theme::loadThemeFromToml(const std::string &filepath,
                                  const std::string &themeName, float dpiScale)
    {
        try
        {
            toml::table config = toml::parse_file(filepath);
            ...
            auto applyThemeConfig = [&](auto &self, const std::string &currentThemeName,
                                        int depth) -> bool
            {
                if (depth > 5)
                ...
                auto themeNodeRaw =
                    findCaseInsensitive(themesNodeRaw->as_table(), currentThemeName);
                if (!themeNodeRaw || !themeNodeRaw->as_table())
                    return false;
                ...
                std::transform(baseThemeLower.begin(), baseThemeLower.end(),
                               baseThemeLower.begin(), ::tolower);
                    ...
                    if (auto styleTable = styleNodeRaw->as_table())
                    {
                        if (auto node = findCaseInsensitive(styleTable, "AntiAliasedLines"))
                            if (auto val = node->value<bool>())
                                style.AntiAliasedLines = *val;
                        if (auto node = findCaseInsensitive(styleTable, "AntiAliasedFill"))
                            if (auto val = node->value<bool>())
                                style.AntiAliasedFill = *val;
                        if (auto node =
                                findCaseInsensitive(styleTable, "AntiAliasedLinesUseTex"))
                            if (auto val = node->value<bool>())
                                style.AntiAliasedLinesUseTex = *val;
                        ...
                        getFloat("CircleTessellationMaxError",
                        ...
                        auto getTreeNodeFlags = [&](const char *key,
                                                    ImGuiTreeNodeFlags &out)
                        {
                            if (auto node = findCaseInsensitive(styleTable, key))
                            {
                                if (auto arr = node->as_array())
                                {
                                    ...
                                    out =
                                        ImGuiTreeNodeFlags_None; // Reset before applying new flags
                                    for (size_t i = 0; i < arr->size(); ++i)
                                    {
                                        if (auto optVal = (*arr)[i].value<std::string>())
                                        {
                                            std::string flagStr = *optVal;
                                            std::transform(flagStr.begin(), flagStr.end(),
                                                           flagStr.begin(), ::tolower);

                                            auto it = flagMap.find(flagStr);
                                            if (it != flagMap.end())
                                            {
                                                out = static_cast<ImGuiTreeNodeFlags>(
                                                    out | it->second); // Bitwise OR to accumulate flags
                                            }
                                        }
                                    }
                                }
                                ...
                                if (auto arr = node->as_array())
                                {
                                    ...
                                    for (size_t i = 0; i < arr->size(); ++i)
                                    {
                                        if (auto optVal = (*arr)[i].value<std::string>())
                                        {
                                            ...
                                            if (it != flagMap.end())
                                            {
                                                out = static_cast<ImGuiHoveredFlags>(
                                                    out | it->second); // Bitwise OR to accumulate flags
                                            }
                                        }
                                    }
                        ...
                        getHoveredFlags("HoverFlagsForTooltipMouse",
                                        style.HoverFlagsForTooltipMouse);
                        getHoveredFlags("HoverFlagsForTooltipNav",
                ...
                if (auto selectorsNodeRaw =
                        findCaseInsensitive(themeTable, "selectors"))
                {
