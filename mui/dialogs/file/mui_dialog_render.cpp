...
namespace mui_dlg
{
    void FileDialog::m_renderTree(FileTreeNode *node)
    {
        ...
        if (FolderNode(displayName.c_str(),
                       (ImTextureID)(uintptr_t)m_getIcon(node->Path, true),
                       isClicked))
        {
            if (!node->Read)
            {
        ...
        if (m_zoom == 1.0f)
        {
                    ...
                    ImVec2(0, -FLT_MIN)))
            {
                ...
                for (auto &entry : m_content)
                {
                    ...
                    {
                        ImGui::TextColored(ImGui::GetStyle().Colors[ImGuiCol_Text], "%s",
                                           entry.IsDirectory ? ICON_FA_FOLDER : ICON_FA_FILE);
                    }
                    ImGui::SameLine();
        ...
        else
        {
            ...
            for (auto &entry : m_content)
            {
                ...
                if (FileIcon(
                        ...
                        entry.HasIconPreview ? (ImTextureID)(uintptr_t)entry.IconPreview
                                             : (ImTextureID)(uintptr_t)m_getIcon(
                                                   entry.Path, entry.IsDirectory),
                        entry.IsDirectory, ImVec2(32 + 16 * m_zoom, 32 + 16 * m_zoom),
    ...
    void FileDialog::m_renderFileDialog()
    {
        ...
        m_favoriteButton->setSelected(std::count(m_favorites.begin(),
                                                 m_favorites.end(),
                                                 m_currentDirectory.string()) > 0);
