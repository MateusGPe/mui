...
namespace mui
{
    ...
    void RangeSlider::renderControl()
    {
        ...
        if (spanAvailWidth) ImGui::PushItemWidth(-FLT_MIN);
        ...
        if (ImGui::SliderFloat2("##range", values, minBound, maxBound, "%.2f")) {
            ...
            currentMin = values[0];
            currentMax = values[1];
            if (onChangedCb) onChangedCb(currentMin, currentMax);
        }

        if (spanAvailWidth) ImGui::PopItemWidth();
    ...
    RangeSliderPtr RangeSlider::setRange(float vMin, float vMax) { 
        currentMin = vMin; currentMax = vMax; return self(); 
    }
    RangeSliderPtr RangeSlider::onChanged(std::function<void(float, float)> cb) { 
        onChangedCb = std::move(cb); return self(); 
    }
}
