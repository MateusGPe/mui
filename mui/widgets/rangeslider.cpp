...
namespace mui
{
    RangeSlider::RangeSlider(float min, float max)
        : minBound(min), maxBound(max), currentMin(min), currentMax(max)
    {
    ...
    void RangeSlider::renderControl()
    {
        ...
        auto render_slider = [&]
        {
            ...
            const float grab_sz =
                ImMin(style.GrabMinSize, frame_bb.GetHeight() - grab_padding * 2.0f);
            ...
            if (held)
            {
                ...
                float new_t = ImClamp(
                    (mouse_x - slider_usable_pos_min) / slider_usable_sz, 0.0f, 1.0f);
            ...
            ImRect highlight_bb(ImVec2(grabMinX, frame_bb.Min.y + grab_padding),
                                ImVec2(grabMaxX, frame_bb.Max.y - grab_padding));
            window->DrawList->AddRectFilled(highlight_bb.Min, highlight_bb.Max,
            ...
            {
                ImRect grab_bb(ImVec2(x - grab_sz * 0.5f, frame_bb.Min.y + grab_padding),
                               ImVec2(x + grab_sz * 0.5f, frame_bb.Max.y - grab_padding));
                ImU32 col = ImGui::GetColorU32(is_active ? ImGuiCol_SliderGrabActive
                                                         : ImGuiCol_SliderGrab);
                window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, col,
                                                style.GrabRounding);
            };
            ...
            {
                char val_buf[32];
                if (active_handle == 1 || (hovered &&
                                           ImFabs(g.IO.MousePos.x - grabMinX) <=
                                               ImFabs(g.IO.MousePos.x - grabMaxX) &&
                                           active_handle == 0))
                {
            ...
            ImGui::RenderTextClipped(frame_bb.Min, frame_bb.Max, bufFull, NULL, NULL,
                                     ImVec2(0.5f, 0.5f));
    ...
    RangeSliderPtr RangeSlider::bind(
        std::shared_ptr<Observable<std::pair<float, float>>> observable)
    {
