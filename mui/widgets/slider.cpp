...
{
  // Slider Int
  SliderInt::SliderInt(int min, int max)
      : minVal(min), maxVal(max), value(min), format("%d")
  {
  ...
  // Slider Float
  SliderFloat::SliderFloat(float min, float max)
      : minVal(min), maxVal(max), value(min), format("%.3f")
  {
    App::assertMainThread();
  }
  void SliderFloat::renderControl()
  {
    ...
    ImGuiSliderFlags flags = logarithmic ? ImGuiSliderFlags_Logarithmic : 0;
    if (ImGui::SliderFloat("##slider", &value, minVal, maxVal, format.c_str(),
                           flags))
    {
      this->onChangedSignal(value);
    }
