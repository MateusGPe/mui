...
namespace mui
{
    ...
    void IconButton::renderControl()
    {
        ...
        if (iconTex || !iconText.empty()) contentWidth += calculatedIconWidth;
        if (!text.empty()) {
            if (contentWidth > 0.0f) contentWidth += style.ItemInnerSpacing.x;
            contentWidth += ImGui::CalcTextSize(text.c_str()).x;
        }

        if (actualSize.x <= 0) actualSize.x = contentWidth + style.FramePadding.x * 2.0f;
        if (actualSize.y <= 0) actualSize.y = std::max((iconTex || !iconText.empty() ? iconSize.y : 0.0f), ImGui::GetFontSize()) + style.FramePadding.y * 2.0f;
        ...
        // 3. Process Events
        bool hovered, held;
        bool pressed = ImGui::ButtonBehavior(bb, window->GetID("##ibtn"), &hovered, &held);

        if (pressed && onClickCb) onClickCb();
        if (hovered && ImGui::IsMouseDoubleClicked(0) && onDoubleClickCb) onDoubleClickCb();

        // 4. Draw Background
        ImU32 col_bg = 0;
        if (held) col_bg = ImGui::GetColorU32(ImGuiCol_HeaderActive);
        else if (hovered) col_bg = ImGui::GetColorU32(ImGuiCol_HeaderHovered);
        else if (selected) col_bg = ImGui::GetColorU32(ImGuiCol_Header);
    ...
    IconButtonPtr IconButton::setIconText(const std::string &t) { iconText = t; return self(); }
