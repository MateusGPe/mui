...
namespace mui
{
  ...
  void Separator::renderCustomSeparator()
  {
    ...
    ImU32 col = useCustomColor ? ImGui::GetColorU32(color)
    ...
    if (orientation == SeparatorOrientation::Horizontal)
    {
      ...
      const ImRect bb(
          window->DC.CursorPos,
          ImVec2(window->DC.CursorPos.x + w, window->DC.CursorPos.y + thickness));
      ...
      if (ImGui::ItemAdd(bb, 0))
      {
        ...
        {
          window->DrawList->AddLine(ImVec2(bb.Min.x, bb.GetCenter().y),
                                    ImVec2(bb.Max.x, bb.GetCenter().y), col,
                                    thickness);
        }
      }
    }
    else // Vertical
    {
      ...
      const ImRect bb(
          window->DC.CursorPos,
          ImVec2(window->DC.CursorPos.x + thickness, window->DC.CursorPos.y + h));
      ImGui::ItemSize(bb.GetSize());
      if (ImGui::ItemAdd(bb, 0))
      {
        if (isRect)
        ...
        {
          window->DrawList->AddLine(ImVec2(bb.GetCenter().x, bb.Min.y),
                                    ImVec2(bb.GetCenter().x, bb.Max.y), col,
                                    thickness);
        }
      }
      ImGui::SameLine(0, style.ItemSpacing.x);
