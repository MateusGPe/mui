...
namespace mui
{
    ...
    void Table::renderControl()
    {
        ...
        if (ImGui::BeginTable("##table", (int)columns.size(), flags, size))
        {
            ...
            {
                ImGuiTableColumnFlags colFlags = col.fixedWidth
            ...
            if (m_flags & ImGuiTableFlags_Sortable)
            {
                if (ImGuiTableSortSpecs *sorts_specs = ImGui::TableGetSortSpecs())
                {
                    ...
                    {
                        onSortRequestedSignal(sorts_specs->Specs->ColumnIndex,
            ...
            for (size_t r = 0; r < rows.size(); ++r)
            {
                ...
                for (size_t c = 0; c < columns.size(); ++c)
                {
                    ...
                    if (c == 0)
                    {
                        ...
                        // FIX: Changed ImGuiSelectableFlags_AllowItemOverlap to
                        // ImGuiSelectableFlags_AllowOverlap
                        if (ImGui::Selectable(selId.c_str(), selectedRow == (int)r,
                    ...
                    if (c < rows[r].size() && rows[r][c])
                    {
                        ...
                        if (needsIndexId)
                            ImGui::PushID(static_cast<int>(r * columns.size() + c));
    ...
    TablePtr Table::addColumn(const std::string &name, float weight,
                              bool fixedWidth)
    {
