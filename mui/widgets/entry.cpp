...
namespace mui
{
  Entry::Entry(const std::string &initialText, bool password, bool multiline,
               float h)
  ...
  Entry::Entry(char *buf, size_t buf_size) : buffer(buf), bufferSize(buf_size)
  {
    App::assertMainThread();
    m_flags = 0;
  }
    ...
    auto render_entry = [&]
    {
      ...
      if (buffer)
      {
        ...
        { /* Not implemented for buffer mode */
          ...
          entered = ImGui::InputTextWithHint("##entry", hint.c_str(), buffer,
                                             bufferSize, flags, callback, this);
        else
      ...
      else
      {
        if (isMultiline)
          changed = ImGui::InputTextMultiline(
              "##entry", &text, ImVec2(spanAvailWidth ? -FLT_MIN : 0, height),
              flags, callback, this);
        else if (!hint.empty())
          ...
          if (clipboard_text)
          {
            ...
            if (!isMultiline)
            {
              if (m_flags & ImGuiInputTextFlags_CharsNoBlank)
                clipboard_str.erase(
                    std::remove_if(clipboard_str.begin(), clipboard_str.end(),
                                   [](char c)
                                   {
                                     return c == '\n' || c == '\r' || c == ' ' ||
                                            c == '\t';
                ...
                std::replace_if(
                    clipboard_str.begin(), clipboard_str.end(),
                    [](char c)
                    { return c == '\n' || c == '\r'; }, ' ');
            }
  ...
  PasswordEntry::PasswordEntry() : Entry()
  {
    m_flags |= ImGuiInputTextFlags_Password;
  }
