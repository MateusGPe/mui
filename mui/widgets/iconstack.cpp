...
namespace mui
{
    ...
    void IconStack::renderControl()
    {
        ...
        // This ensures all icons are centered relative to the same width, preventing
        // visual shifts.
        float max_icon_width = 0.0f;
        for (const auto &icon : icons)
        {
            max_icon_width =
                std::max(max_icon_width, ImGui::CalcTextSize(icon.icon.c_str()).x);
        }
        ...
        ImGui::BeginGroup(); // Group the buttons to treat them as a single item for
                             // layout
        for (size_t i = 0; i < icons.size(); ++i)
        {
            ...
            const ImVec2 icon_pos(bb.Min.x + (button_width - icon_size.x) * 0.5f,
                                  bb.Min.y + (button_height - icon_size.y) * 0.5f);

            ScopedColor text_color;
            if (icons[i].text_hover)
                text_color.push(ImGuiCol_Text,
                                ImGui::GetColorU32(held      ? ImGuiCol_TextDisabled
