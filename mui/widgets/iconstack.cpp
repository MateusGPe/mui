...
namespace mui
{
    ...
    void IconStack::renderControl()
    {
        ...
        // Calculate single button natural boundaries and clamp to user-defined Min/Max
        ...
        float natural_w = (base_btn_w * icons.size()) + (style.ItemInnerSpacing.x * (icons.size() - 1));
        ...
        // Evenly divide the constrained width among all buttons, subtracting the spacing.
        float final_btn_w = std::max(1.0f, (final_size.x - (style.ItemInnerSpacing.x * (icons.size() - 1))) / icons.size());
        ...
        if (m_scaleToFit)
        {
            float target_text_w = std::max(1.0f, final_btn_w - style.FramePadding.x * 2.0f);
            float target_text_h = std::max(1.0f, final_btn_h - style.FramePadding.y * 2.0f);
        ...
        for (size_t i = 0; i < icons.size(); ++i)
        {
            ...
            ImVec2 icon_size = font->CalcTextSizeA(effective_font_size, FLT_MAX, 0.0f, icons[i].icon.c_str());
            ...
            window->DrawList->AddText(font, effective_font_size, icon_pos, text_col, icons[i].icon.c_str());
        ...
        renderTooltip(); // Ensure parent Control tooltip capability is fired off if set
