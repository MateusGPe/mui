...
namespace mui
{
    ...
    void PropertyGrid::renderControl()
    {
        ...
        {
            ImGui::TableSetupColumn("Property", ImGuiTableColumnFlags_WidthFixed, nameColumnWidth);
    ...
    PropertyGridPtr PropertyGrid::addCategory(const std::string &name, bool defaultOpen)
    {
        verifyState();
    ...
    PropertyGridPtr PropertyGrid::addProperty(const std::string &label, IControlPtr editor)
    {
        verifyState();
        if (categories.empty())
        {
        ...
        // (assuming the target widget respects spanAvailWidth or uses -FLT_MIN context sizing)
