...
namespace mui
{
  PropertyGrid::PropertyGrid()
  {
    ...
    inlineShadowEnabled = false; // PropertyGrid typically doesn't have its own shadow
    // Initialize new members to default/sentinel values
    // nameColumnWidth, valueColumnWeight, itemSpacingY, categorySpacingY, itemIndent
    // are already initialized in .hpp or have meaningful defaults.
  }
  ...
  void PropertyGrid::renderControl()
  {
    ...
    if (itemSpacingY >= 0) {
        itemSpacingOverride.push(ImGuiStyleVar_ItemSpacing, ImVec2(ImGui::GetStyle().ItemSpacing.x, itemSpacingY));
    }
    ...
    if (ImGui::BeginTable("##property_grid", 2, flags))
    {
      ...
      for (size_t c = 0; c < categories.size(); ++c)
      {
        ...
        if (!cat.name.empty())
        {
          ...
          std::string headerLabel = cat.icon.empty() ? cat.name : cat.icon + " " + cat.name;
        ...
        if (nodeOpen)
        {
          for (size_t i = 0; i < cat.items.size(); ++i)
          {
            ...
            if (!cat.name.empty())
            {
                ImGui::Indent(itemIndent >= 0 ? itemIndent : ImGui::GetTreeNodeToLabelSpacing());
            }
            ...
            ImGui::TextUnformatted(item.label.c_str()); // Use TextUnformatted for raw string

            if (!cat.name.empty())
            {
                ImGui::Unindent(itemIndent >= 0 ? itemIndent : ImGui::GetTreeNodeToLabelSpacing());
            }
  ...
  PropertyGridPtr PropertyGrid::addCategory(const std::string &name, const std::string &icon, ImVec4 color, ImGuiTreeNodeFlags flags)
  {
    verifyState();
    ...
    newCat.icon = icon;
    newCat.flags = flags;
    if (color.w > 0) { // Check if color is explicitly set (alpha > 0)
        newCat.color = color;
  ...
  PropertyGridPtr PropertyGrid::addProperty(const std::string &label,
                                            IControlPtr editor, ImVec4 labelColor)
  {
    verifyState();
    if (categories.empty())
    {
      // If no category was explicitly created, create a blank, headerless group.
      categories.push_back({"", "", ImVec4(0,0,0,0), false, {}, ImGuiTreeNodeFlags_DefaultOpen});
    }
    ...
    if (labelColor.w > 0) { // Check if color is explicitly set (alpha > 0)
  ...
  {
      verifyState();
      if (v) m_flags |= PropertyGridFlags_Visible;
      else m_flags &= ~PropertyGridFlags_Visible;
  ...
  {
      verifyState();
      if (e) m_flags |= PropertyGridFlags_Enabled;
      else m_flags &= ~PropertyGridFlags_Enabled;
  ...
  {
      verifyState();
      if (span) m_flags |= PropertyGridFlags_SpanCategoryHeaders;
      else m_flags &= ~PropertyGridFlags_SpanCategoryHeaders;
      return self();
  }
  ...
  {
      verifyState();
      if (show) m_flags |= PropertyGridFlags_ZebraStripes;
      else m_flags &= ~PropertyGridFlags_ZebraStripes;
      return self();
  }
} // namespace mui