...
namespace mui
{
    ...
    SplitterViewPtr SplitterView::onSplitRatioChanged(std::function<void(float)> cb)
    {
        if (cb)
    ...
    SplitterViewPtr SplitterView::bindSplitRatio(std::shared_ptr<Observable<float>> observable)
    {
        setSplitRatio(observable->get());
        m_connections.push_back(observable->onValueChanged.connect([this](const float &val) { mui::App::queueMain([this, val]() { this->setSplitRatio(val); }); }));
        m_connections.push_back(onSplitRatioChangedSignal.connect([observable](float val) { observable->set(val); }));
        return self();
    }
} // namespace mui
