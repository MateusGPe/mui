...
namespace mui
{
  TreeNode::TreeNode(const std::string &label) : label(label)
  {
  ...
  void TreeNode::renderControl()
  {
    ...
    bool nodeOpen =
        ImGui::TreeNodeEx(label.c_str(), flags, "%s", displayStr.c_str());
    ...
    if (iconTex)
    {
      ...
      float iconX = min.x + ImGui::GetTreeNodeToLabelSpacing() - fontSize -
                    ImGui::GetStyle().ItemInnerSpacing.x;
      float iconY = min.y + (ImGui::GetItemRectSize().y - fontSize) * 0.5f;

      ImGui::GetWindowDrawList()->AddImage(
          iconTex, ImVec2(iconX, iconY),
          ImVec2(iconX + fontSize, iconY + fontSize));
    }
  ...
  TreeNodePtr TreeNode::setIconText(const std::string &t)
  {
    iconText = t;
    return self();
  }
  TreeNodePtr TreeNode::setIconTexture(ImTextureID tex)
  {
    iconTex = tex;
    return self();
  }
  ...
  {
    return m_flags & ImGuiTreeNodeFlags_Selected;
  }
  TreeNodePtr
  TreeNode::bindSelected(std::shared_ptr<Observable<bool>> observable)
  {
    ...
    // Two-way binding requires external logic to manage selection state across
    // multiple nodes. The app can use the onClick signal to update the
    // observable.
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTreeNodeFlags_Framed; // CollapsingHeader is essentially a
                                            // framed TreeNode with no tree push
    else
      m_flags &= ~ImGuiTreeNodeFlags_Framed;
    return self();
  }
