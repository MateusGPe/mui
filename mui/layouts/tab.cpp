...
namespace mui
{
      ...
      for (size_t i = 0; i < pages.size(); ++i)
      {
        if (ImGui::BeginTabItem(pages[i].name.c_str()))
        {
          ...
          if (pages[i].margined)
          {
            ...
            ImGui::BeginChild("##margined_tab_content", ImVec2(0, 0),
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_AutoSelectNewTabs;
    else
      m_flags &= ~ImGuiTabBarFlags_AutoSelectNewTabs;
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_NoCloseWithMiddleMouseButton;
    else
      m_flags &= ~ImGuiTabBarFlags_NoCloseWithMiddleMouseButton;
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_NoTabListScrollingButtons;
    else
      m_flags &= ~ImGuiTabBarFlags_NoTabListScrollingButtons;
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_NoTooltip;
    else
      m_flags &= ~ImGuiTabBarFlags_NoTooltip;
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_FittingPolicyScroll;
    else
      m_flags &= ~ImGuiTabBarFlags_FittingPolicyScroll;
    return self();
  }
  ...
  {
    if (b)
      m_flags |= ImGuiTabBarFlags_FittingPolicyResizeDown;
    else
      m_flags &= ~ImGuiTabBarFlags_FittingPolicyResizeDown;
    return self();
  }
